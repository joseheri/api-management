...
## Claim Identifier / Email Field

The claim workflow uses an approved email-like value as a claim identifier.

Important:

- This is not an email delivery feature.
- The system will not send emails.
- The system will not have SMTP access.
- The system will not send magic links, claim links, reset links, or verification codes.
- The claim code is generated by the system and manually delivered out-of-band by an internal/admin process.
- The client/user claims the API key by entering:
  - approved email / claim identifier
  - claim code

Naming guidance:

- UI label may say: "Approved Email / Claim Identifier"
- DTO field may be named `claimIdentifier` or `approvedEmail`
- If using `approvedEmail`, comments and documentation must state that no email is sent.
